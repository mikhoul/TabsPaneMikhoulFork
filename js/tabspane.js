...
Foreground = {
  ...
  _setLayoutWidth: function() {
    ...
    var tabOuterWidth = 100, //see tabspane.css .tabOuter width  350
...
//      distance: 5,
//      distance: 5,
     /*  cancel: '.tabDescription', */  /* Permet de dragger la div description ou  non */
      scroll: false,
  ...
  initMultiselect: function() {
    ...
    this.selectionInstance = $('.appleOddRow').multiselect({
      ...
      cancelSelectors: ['div.tabCapture', '#tabMenu', 'menuitem', '.tabDescription',], /* selecteurs qui ne seront pas highlighté en draggant  */  
  ...
  initContextMenu: function() {
    Menu.fill([
      {
        label: 'Move Selected Tabs to a new window',
          ...
          case 'status':
            if (changeInfo.status == 'complete') {
              ...
              if ($(tabThumb).find('.tabDescription').html() != 'Tabs Pane')
                $(tabThumb).find('.tabDescription').html('{0} {1} {2} {3} {4}'.format("<b id=url_title>", tab.title, "</b><p id=url_description>", tab.url, "</p>"));   /* tab.title, tab.url */
            }
            break;
          case 'url':
            $(tabThumb).find('.tabDescription').html('{0} {1} {2} {3} {4}'.format("<b id=url_title>", tab.title, "</b><p id=url_description>", tab.url, "</p>"));  /* tab.title, tab.url */
            break;
          case 'favIconUrl':
            $(tabThumb).find('.tabIcon').attr('src', 'chrome://favicon/size/50/' + tab.url);   
  ...
  render: function(tab) {
    var skeleton = $(
      '<div class="tabOuter">' + // I have to append this div, otherwise jQuery doesn't see .tabThumb until it's appended
        '<div class="tabThumb">' +
          '<div class="tabCapture"></div>' +
           '<div class="tabFooter"><div class="tabCloseButton"></div>' +    
            '<img class="tabIcon" />' + 
    ...
    //icon & title
    var smallIcon = tab.favIconUrl ? "chrome://favicon/size/50/" + tab.url : chrome.extension.getURL('img/tab.png');
    ...
    if (Foreground.isExtensionURL(tab.url)) {
      ...
      /* captureImage = chrome.extension.getURL('img/capture.png'); */     /* DISABLED BLOCK SINCE NO LONGER NEED TAB CAPTURE/SCREENSHOT */
    } else {
    //set all other tabs icons and captures
      skeleton.find('.tabIcon').attr('src', favIconUrl);                                                
      skeleton.find('.tabDescription').html('{0} {1} {2} {3} {4}'.format("<b id=url_title>", tab.title, "</b><p id=url_description>", tab.url, "</p>"));    /* tab.title, tab.url */
      /* captureImage = chrome.extension.getBackgroundPage().tabCaptures[tab.id]; */   /* DISABLED BLOCK SINCE NO LONGER NEED TAB CAPTURE/SCREENSHOT */
    }

    //capture  Décide si screenshot ou FavIcon

    var tabCapture = skeleton.find('.tabCapture');
    if (captureImage != null) {
      tabCapture.css({                //  CSS avec Screenshot               
          /* 'background': 'url({0})'.format(captureImage),  */        /* DISABLED BLOCK SINCE NO LONGER NEED TAB CAPTURE/SCREENSHOT */
          'background-size': 'cover'
        });
    } else {
      // default tab capture image - CSS avec FavIcon
      tabCapture.css({
    /*     'background-image': 'url("{0}"), radial-gradient(#ddd, #fff 93%, #fff 18%)'.format(largeIcon), */    /*  Mike */
    ...
    $('.tabOuter').each(function(index, item) {
      if (search != '') {
        if ($(this).find('.tabDescription').text().toLowerCase().match(search) == null) {
          ...
          $(this).css({display:'-webkit-inline-box'});    /*-webkit-inline-box: Pour avoir les thumbs collé verticalement - Ori: inline-block */ 
          visibleCount++;
        }
      } else {
